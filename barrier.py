...
def main():
    ...
    source_file_name = f'Neurips21/{save_dir}/Train/{model1_id}/{destination_blob_name}'
    ...
    destination_blob_name = 'model_best.th'
    source_file_name = f'Neurips21/{save_dir}/Train/{model2_id}/{destination_blob_name}'
    download_blob(bucket_name, source_file_name, destination_blob_name)
    ...
    barrier_test = dict_before['barrier_test']
    lmc_test = dict_before['test_lmc']

    test_avg_org_models = dict_before['test_avg_models'][0]
    train_avg_org_models = dict_before['train_avg_models'][0]
    print("barrier_test_original", barrier_test)
    print("lmc_test_original", lmc_test)

    source_file_name = f'dict_before_{args.pair}.pkl'
    destination_blob_name = f'Neurips21/{save_dir}/barrier/original/{source_file_name}'
    pickle_out = pickle.dumps(dict_before)
    upload_pkl(bucket_name, pickle_out, destination_blob_name)
    ########################################## oracle barrier
    bucket_name = 'permutation-mlp'
    destination_blob_name = 'model_best.th'
    source_file_name = f'Neurips21/{save_dir}/Train/{17}/{destination_blob_name}'
    download_blob(bucket_name, source_file_name, destination_blob_name)
    ...
    destination_blob_name = f'Neurips21/{save_dir}/barrier/oracle/before/{source_file_name}'
    ...
    #
    # print("barrier_test_SA_oracle", barrier_test)
    # print("lmc_test_SA_oracle", lmc_test)
    #
    # # source_file_name = f'dict_after_pair{pair}_{args.rand_seed}.pkl'
    # source_file_name = f'dict_after_pair{pair}.pkl'
    # # destination_blob_name = f'Neurips21/{save_dir}/barrier/oracle/SA/auto/{source_file_name}'
    # destination_blob_name = f'Neurips21/{save_dir}/barrier/oracle/SA/grid/{exp_no}/{source_file_name}'
    ...
    # destination_blob_name = f'Neurips21/{save_dir}/barrier/SA/grid/{exp_no}/{source_file_name}'
    # pickle_out = pickle.dumps(dict_after)
    # upload_pkl(bucket_name, pickle_out, destination_blob_name)
...
def barrier_SA(arch, model, sd1, sd2, w2, init_state, tmax, tmin, steps, train_inputs, train_targets, train_avg_org_models, nchannels, nclasses, nunits):
    ...
    class BarrierCalculationProblem(Annealer):
        ...
        def energy(self):
            ...
            result = evaluate_model_small(model, train_inputs, train_targets)['top1']
